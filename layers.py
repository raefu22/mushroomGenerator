...
def showParameter(*args):
    showCheckbox = cmds.checkBox(checkbox, q = True, v = True)
    cmds.checkBoxGrp(show, edit=True, enable=True)
   
def hideParameter(*args):
    showCheckbox = cmds.checkBox(checkbox, q = True, v = False)
    cmds.checkBoxGrp(show, edit=True, enable=False)
#checkbox_value
#UI
...
checkbox = cmds.checkBox(label="Show Parameter", changeCommand=showParameter, ofc = hideParameter)

smooth = cmds.checkBoxGrp("smooth", q = True, v1=True)


show = cmds.checkBoxGrp("layers", numberOfCheckBoxes=1, label='Save a LP Copy ', v1=False)
cmds.checkBoxGrp(show, edit=True, enable=False)
...
def createMushroom():
    ...
    #display layers
    cmds.createDisplayLayer(n="LowPoly")
    cmds.createDisplayLayer(n="HighPoly")
    ...
    for x in range(1, num+1):
        ...
        if (smooth == True):
            ...
            #cmds.editDisplayLayerMembers('HighPoly', mushroom)
